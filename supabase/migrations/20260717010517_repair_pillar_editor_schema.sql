-- The original pillar-editor migration is present in remote migration history,
-- but its schema changes are missing from the production database. Reapply the
-- complete feature idempotently under a new migration version.
...
create index if not exists profiles_pillar_id_idx on public.profiles (pillar_id)
...
drop policy if exists "assigned pillar editors can update pillars" on public.site_content;
